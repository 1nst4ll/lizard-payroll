...
const props = defineProps({
    employee: Object,
    departments: Array,
    jobRoles: Array, // Job roles filtered by employee's department
});
...
const form = useForm({
    ...
    nib_number: props.employee.nib_number,
    nib_deduction_override: props.employee.nib_deduction_override,
    nhib_number: props.employee.nhib_number,
    nhib_deduction_override: props.employee.nhib_deduction_override,
    payment_method: props.employee.payment_method,
    starting_date: props.employee.starting_date,
    contract_type: props.employee.contract_type,
    rate: props.employee.rate,
    contract_signed: props.employee.contract_signed,
    uniform_size: props.employee.uniform_size,
    department_id: props.employee.department_id,
    job_role_id: props.employee.job_role_id,
});

const currentJobRoles = ref(props.jobRoles);

const fetchJobRolesForDepartment = async (departmentId) => {
    try {
        const url = departmentId ? `/api/departments/${departmentId}/job-roles` : '/api/job-roles';
        const response = await axios.get(url);
        currentJobRoles.value = response.data;
    } catch (err) {
        console.error('Failed to fetch job roles for department:', err);
    }
};
...
watch(() => form.department_id, (newVal, oldVal) => {
    if (newVal !== oldVal) {
        ...
        fetchJobRolesForDepartment(newVal);
    }
});
                    ...
                    <form @submit.prevent="submit">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="department_id" v-model="form.department_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                            ...
                            <div>
                                ...
                                <select id="job_role_id" v-model="form.job_role_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                    ...
                                    <option v-for="jobRole in currentJobRoles" :key="jobRole.id" :value="jobRole.id">{{ jobRole.title }}</option>
                                </select>
